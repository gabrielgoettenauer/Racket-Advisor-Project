...
>
      <div class="hero-top">
        <h1>
          Atleta, encontramos sua <span>raka ideal!</span>
        </h1>
      ...
      <div class="hero-content">
        <div class="racket-visual">
          ...
          <img class="racket-image" :src="mainImage" :alt="mainRacket.name" />
        ...
        <div class="spec-card">
          ...
          <div class="spec-grid">
            <div class="spec-column">
              <div class="spec-item" v-for="item in mainSpecsLeft" :key="item.label">
            ...
            <div class="spec-column">
              <div class="spec-item" v-for="item in mainSpecsRight" :key="item.label">
                <div class="spec-icon">{{ item.icon }}</div>
                <div>
                  <strong>{{ item.label }}</strong>
                  <p>{{ item.value }}</p>
          ...
          <div class="action-row">
            ...
            <button class="site-button">Ver Site AMA →</button>
          </div>
        </div>
      </div>
    </section>
...
>
      <div class="hero-top">
        <h1>
          Sua <span>segunda opção</span>
        </h1>

        <p class="subtitle">Outra excelente escolha para o seu perfil</p>
      </div>

      <div class="hero-content">
        <div class="racket-visual">
          <div class="racket-card-bg"></div>
          <img class="racket-image second" :src="secondImage" :alt="secondRacket?.name" />
          <div class="racket-base"></div>
        </div>
        ...
        <div class="spec-card">
          ...
          <div class="spec-grid">
            <div class="spec-column">
              <div class="spec-item" v-for="item in secondSpecsLeft" :key="item.label">
                <div class="spec-icon">{{ item.icon }}</div>
                <div>
                  <strong>{{ item.label }}</strong>
                  <p>{{ item.value }}</p>
            ...
            <div class="spec-column">
              <div class="spec-item" v-for="item in secondSpecsRight" :key="item.label">
                <div class="spec-icon">{{ item.icon }}</div>
                <div>
                  <strong>{{ item.label }}</strong>
                  <p>{{ item.value }}</p>
          ...
          <div class="action-row">
            <button class="site-button secondary-btn">Ver Site AMA →</button>
          </div>
        </div>
      </div>
    </section>
...
function buildSpecs(racket) {
  ...
  return {
    left: [
      ...
      { icon: '☰', label: 'ESPESSURA', value: racket.thickness_mm + ' mm' },
    ],
    right: [
...
.racket-card-bg {
  ...
  width: 320px;
  height: 420px;
  background: linear-gradient(180deg, rgba(232, 240, 250, 0.92), rgba(214, 226, 243, 0.92));
